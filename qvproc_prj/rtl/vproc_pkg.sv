...
typedef enum logic [4:0] {
    ...
    ELEM_VREDMAXU,
    ELEM_VREDMAX,
    // HiSEPQ custom quantum instruction: single-qubit style op over vector elements.
    ELEM_QSINGLE,
    // HiSEPQ custom quantum instruction: pairwise op over two vector operands.
    ELEM_QPAIR,
    // HiSEPQ custom quantum instruction: global rotation amount from scalar register.
    ELEM_QROTG,
    // HiSEPQ custom quantum instruction: per-element rotation amount from vector register.
    ELEM_QROTV
} opcode_elem;
...
typedef struct packed {
    ...
    logic       xreg;
`ifdef VPROC_OP_MODE_UNION
    // Reduced by one bit because opcode_elem widened to 5 bits for quantum operations.
    logic [4:0] unused;
`endif
} op_mode_elem;
