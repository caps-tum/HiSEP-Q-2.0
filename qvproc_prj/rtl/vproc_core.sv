...
module vproc_core import vproc_pkg::*; #(
    ...
    typedef struct packed {
        ...
        logic [31:0]         quantum_elem2_raw;
        logic [31:0]         quantum_elem3_raw;
        op_unit              unit;
        op_mode              mode;
    ...
    always_comb begin
        ...
        if (xif_commit_if.commit_valid & (
            ...
            if (~measure_pause_i & dec_buf_valid_q &
                (dec_data_q.unit == UNIT_CFG) &
                (dec_data_q.id == xif_commit_if.commit.id)
            ) begin
        ...
        if (~measure_pause_i & dec_buf_valid_q & (dec_data_q.unit == UNIT_CFG) & (
    ...
    logic queue_dispatch_valid;
    assign queue_dispatch_valid = queue_valid_q & ~measure_pause_i;
    ...
    vproc_dispatcher #(
        ...
        .async_rst_ni       ( async_rst_n        ),
        .sync_rst_ni        ( sync_rst_n         ),
        .instr_valid_i      ( queue_dispatch_valid ),
    ...
    /*
     * Each pipeline can optionally host an ELEM unit. These arrays collect the
     * per-lane quantum slice traces so the core can expose one consolidated
     * stream to `vproc_top` without disturbing the existing datapath.
     */
    logic [PIPE_CNT-1:0]                               pipe_quantum_valid; // quantum qvproc
    logic [PIPE_CNT-1:0][4:0]                          pipe_quantum_op; // quantum qvproc
    logic [PIPE_CNT-1:0][XIF_ID_W-1:0]                 pipe_quantum_instr_id; // quantum qvproc
    logic [PIPE_CNT-1:0][4:0]                          pipe_quantum_vd_addr; // quantum qvproc
    logic [PIPE_CNT-1:0][31:0]                         pipe_quantum_elem1; // quantum qvproc
    logic [PIPE_CNT-1:0][31:0]                         pipe_quantum_elem2; // quantum qvproc
    logic [PIPE_CNT-1:0][31:0]                         pipe_quantum_elem3; // quantum qvproc
    cfg_lmul  [PIPE_CNT-1:0]                           pipe_quantum_lmul; // quantum qvproc
    cfg_emul  [PIPE_CNT-1:0]                           pipe_quantum_emul; // quantum qvproc
    logic [PIPE_CNT-1:0][2:0]                          pipe_quantum_count_mul; // quantum qvproc
    logic [PIPE_CNT-1:0]                               pipe_quantum_first_cycle; // quantum qvproc
    logic [PIPE_CNT-1:0]                               pipe_quantum_last_cycle; // quantum qvproc
    logic [PIPE_CNT-1:0]                               pipe_quantum_vl_part_0; // quantum qvproc
    logic [PIPE_CNT-1:0][4:0]                          pipe_quantum_res_vaddr; // quantum qvproc
    logic [PIPE_CNT-1:0]                               pipe_quantum_first_valid_result; // quantum qvproc
    logic [PIPE_CNT-1:0]                               pipe_quantum_has_valid_result; // quantum qvproc
    logic [PIPE_CNT-1:0][2:0]                          pipe_quantum_qrotv_op2_chunk; // quantum qvproc
...
`endif
                ...
                .quantum_valid_o          ( pipe_quantum_valid     [i] ), // quantum qvproc
                .quantum_op_o             ( pipe_quantum_op        [i] ), // quantum qvproc
                .quantum_instr_id_o       ( pipe_quantum_instr_id  [i] ), // quantum qvproc
                .quantum_vd_addr_o        ( pipe_quantum_vd_addr   [i] ), // quantum qvproc
                .quantum_elem1_o          ( pipe_quantum_elem1     [i] ), // quantum qvproc
                .quantum_elem2_o          ( pipe_quantum_elem2     [i] ), // quantum qvproc
                .quantum_elem3_o          ( pipe_quantum_elem3     [i] ), // quantum qvproc
                .quantum_lmul_o           ( pipe_quantum_lmul      [i] ), // quantum qvproc
                .quantum_emul_o           ( pipe_quantum_emul      [i] ), // quantum qvproc
                .quantum_count_mul_o      ( pipe_quantum_count_mul [i] ), // quantum qvproc
                .quantum_first_cycle_o    ( pipe_quantum_first_cycle[i] ), // quantum qvproc
                .quantum_last_cycle_o     ( pipe_quantum_last_cycle [i] ), // quantum qvproc
                .quantum_vl_part_0_o      ( pipe_quantum_vl_part_0  [i] ), // quantum qvproc
                .quantum_res_vaddr_o      ( pipe_quantum_res_vaddr  [i] ), // quantum qvproc
                .quantum_first_valid_result_o ( pipe_quantum_first_valid_result[i] ), // quantum qvproc
                .quantum_has_valid_result_o   ( pipe_quantum_has_valid_result  [i] ), // quantum qvproc
                .quantum_qrotv_op2_chunk_o    ( pipe_quantum_qrotv_op2_chunk   [i] ) // quantum qvproc
            );
    ...
    /*
     * Export the first active quantum slice trace from the pipeline set. The
     * default configuration has a single ELEM-capable lane, and this mux keeps
     * the top-level interface scalar while still preserving all per-slice
     * information for the active quantum instruction.
     */
    always_comb begin
        quantum_valid_o       = 1'b0; // quantum qvproc
        quantum_op_o          = '0; // quantum qvproc
        quantum_instr_id_o    = '0; // quantum qvproc
        quantum_vd_addr_o     = '0; // quantum qvproc
        quantum_elem1_o       = '0; // quantum qvproc
        quantum_elem2_o       = '0; // quantum qvproc
        quantum_elem3_o       = '0; // quantum qvproc
        quantum_lmul_o        = cfg_lmul'('0); // quantum qvproc
        quantum_emul_o        = cfg_emul'('0); // quantum qvproc
        quantum_count_mul_o   = '0; // quantum qvproc
        quantum_first_cycle_o = '0; // quantum qvproc
        quantum_last_cycle_o  = '0; // quantum qvproc
        quantum_vl_part_0_o   = '0; // quantum qvproc
        quantum_res_vaddr_o   = '0; // quantum qvproc
        quantum_first_valid_result_o = '0; // quantum qvproc
        quantum_has_valid_result_o   = '0; // quantum qvproc
        quantum_qrotv_op2_chunk_o    = '0; // quantum qvproc
        for (int i = 0; i < PIPE_CNT; i++) begin
            if (~quantum_valid_o && pipe_quantum_valid[i]) begin
                quantum_valid_o       = pipe_quantum_valid      [i]; // quantum qvproc
                quantum_op_o          = pipe_quantum_op         [i]; // quantum qvproc
                quantum_instr_id_o    = pipe_quantum_instr_id   [i]; // quantum qvproc
                quantum_vd_addr_o     = pipe_quantum_vd_addr    [i]; // quantum qvproc
