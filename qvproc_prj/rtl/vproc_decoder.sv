                ...
                // arithmetic instructions:
                    ...
                    unique case ({instr_i[31:26], instr_i[14:12]})
                        ...
                        {6'b111100, 3'b000}: begin  // Legacy QV.SINGLE placeholder space (e.g. 0x78/0x79)
                            ...
                            mode_o.elem.xreg   = 1'b0;
                            // QV.SINGLE uses funct7 for GateID, so bit 25 is no longer the standard
                            // RVV vm bit. Do not reinterpret GateID[0] as a v0 mask enable.
                        ...
                        {6'b110010, 3'b000}: begin  // QV.SINGLE fixed gate space (H gate = 7'h64)
                            if (instr_i[31:25] == 7'h64) begin
                                unit_o             = UNIT_ELEM;
                                mode_o.elem.op     = ELEM_QSINGLE;
                                mode_o.elem.xreg   = 1'b0;
                                mode_o.elem.masked = 1'b0;
                        ...
                        {6'b110100, 3'b000}: begin  // QV.SINGLE(MEASURE) fixed gate space (MEASURE = 7'h68)
                            if (instr_i[31:25] == 7'h68) begin
                                unit_o             = UNIT_ELEM;
                                mode_o.elem.op     = ELEM_QSINGLE;
                                mode_o.elem.xreg   = 1'b0;
                                mode_o.elem.masked = 1'b0;
                        ...
                        {6'b111101, 3'b000}: begin  // Legacy QV.PAIR placeholder space (e.g. 0x7A/0x7B)
                            // Keep the legacy placeholder pair-gate encodings alive for existing
                            // exploratory tests, but prefer the fixed CNOT-gate encoding below.
                            unit_o             = UNIT_ELEM;
                        ...
                        {6'b110011, 3'b000}: begin  // QV.PAIR fixed gate space (CNOT gate = 7'h66)
                            if (instr_i[31:25] == 7'h66) begin
                                unit_o             = UNIT_ELEM;
                                mode_o.elem.op     = ELEM_QPAIR;
                                mode_o.elem.xreg   = 1'b0;
                                mode_o.elem.masked = 1'b0;
                        ...
                        {6'b111110, 3'b100}: begin  // QV.ROT.G
                            // Route through ELEM because rotation width depends on current EEW/SEW.
                            unit_o             = UNIT_ELEM;
                            // Select custom global-rotation opcode inside ELEM.
                            mode_o.elem.op     = ELEM_QROTG;
                            // Result is vector data, no xreg writeback path.
                            mode_o.elem.xreg   = 1'b0;
                            // Preserve vm-based masking semantics for this operation.
                            mode_o.elem.masked = instr_masked;
                            // Use vs1 as target/source vector for the rotation.
                            rs2_o.vreg         = 1'b1;
                            // Map vs1 index into ELEM elem1 operand slot.
                            rs2_o.r.vaddr      = instr_vs1;
                            // Disable rs1 vector because scalar angle comes from x register.
                            rs1_o.vreg         = 1'b0;
                            // Enable scalar x-register source for global angle input.
                            rs1_o.xreg         = 1'b1;
                            // Read angle from architectural rs2 x-register value (x_rs2_i).
                            rs1_o.r.xval       = x_rs2_i;
                        end
                        {6'b111111, 3'b011}: begin  // QV.ROT.V
                            // Route through ELEM for per-element rotation amount handling.
                            unit_o             = UNIT_ELEM;
                            // Select custom variable-rotation opcode inside ELEM.
                            mode_o.elem.op     = ELEM_QROTV;
                            // Result is vector data, no scalar writeback.
                            mode_o.elem.xreg   = 1'b0;
                            // Preserve vm-based masking semantics for this operation.
                            mode_o.elem.masked = instr_masked;
                            // Use vs1 as target/source vector for element data.
                            rs2_o.vreg         = 1'b1;
                            // Map vs1 index into ELEM elem1 operand slot.
                            rs2_o.r.vaddr      = instr_vs1;
                            // Use vector rs1 slot to carry angle vector source.
                            rs1_o.vreg         = 1'b1;
                            // Disable scalar xreg interpretation for rs1.
                            rs1_o.xreg         = 1'b0;
                            // Provide vs2 index as per-element angle vector source.
                            rs1_o.r.vaddr      = instr_vs2;
                            // QRV uses mixed-width operands: vs1 exports 8-bit indices while vs2
                            // exports 32-bit angles. With VLEN=128 and a maximum architectural
                            // LMUL of m8, one-to-one angle mapping is only physically possible for
                            // mf2/m1/m2 on the 8-bit index side. Keep m4/m8 illegal so the
                            // instruction never advertises unsupported capacity. // quantum qvproc
                            unique case (lmul_i) // quantum qvproc
