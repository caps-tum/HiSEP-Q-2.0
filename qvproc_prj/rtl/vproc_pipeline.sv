...
module vproc_pipeline import vproc_pkg::*; #(
    ...
    typedef struct packed {
        ...
        cfg_lmul                         lmul;           // program-level LMUL // quantum qvproc
        cfg_emul                         emul;           // effective MUL factor
        cfg_vxrm                         vxrm;
        logic        [CFG_VL_W     -1:0] vl;
        logic                            vl_0;
        logic                     [31:0] quantum_elem2_raw; // quantum qvproc
        logic                     [31:0] quantum_elem3_raw; // quantum qvproc
        logic                            qrotv_startup; // quantum qvproc
        logic                     [1 :0] qrotv_startup_phase; // quantum qvproc
        logic                     [2 :0] qrotv_op2_chunk; // quantum qvproc
        logic                     [31:0] xval;
    ...
    logic        state_ready;
    logic        qrotv_startup_done; // quantum qvproc
    logic        qrotv_op2_ready; // quantum qvproc
    ...
    always_comb begin
        ...
        if (pipe_in_ready_o) begin
            ...
            state_next.lmul                    = pipe_in_state_i.lmul; // quantum qvproc
            state_next.eew                     = pipe_in_state_i.eew;
            state_next.emul                    = pipe_in_state_i.emul;
            state_next.vxrm                    = pipe_in_state_i.vxrm;
            state_next.vl                      = pipe_in_state_i.vl;
            state_next.vl_0                    = pipe_in_state_i.vl_0;
            state_next.quantum_elem2_raw       = pipe_in_state_i.quantum_elem2_raw; // quantum qvproc
            state_next.quantum_elem3_raw       = pipe_in_state_i.quantum_elem3_raw; // quantum qvproc
            state_next.qrotv_startup           = ((pipe_in_state_i.unit == UNIT_ELEM) &&
                                                  (pipe_in_state_i.mode.elem.op == ELEM_QROTV)); // quantum qvproc
            state_next.qrotv_startup_phase     = 2'b00; // quantum qvproc
            state_next.qrotv_op2_chunk         = ((pipe_in_state_i.unit == UNIT_ELEM) &&
                                                  (pipe_in_state_i.mode.elem.op == ELEM_QROTV)) ?
                                                 3'b111 : 3'b000; // quantum qvproc
    ...
    always_comb begin
        ...
        end else if ((OP_DYN_ADDR == '0) | (state_q.aux_count == '1)) begin
    ...
    logic [1:0]            qrotv_startup_phase_next; // quantum qvproc
    always_comb begin
        qrotv_startup_phase_next = state_q.qrotv_startup_phase + 2'd1; // quantum qvproc
        for (int i = 0; i < OP_CNT; i++) begin
    ...
    always_comb begin
        ...
        for (int i = 0; i < OP_CNT; i++) begin
            ...
            // (which may be non-zero if the auxiliary counter is not used by the new instruction)
                ...
                if (~OP_MASK[i]) begin
                    if ((state_next.unit == UNIT_ELEM) && (state_next.mode.elem.op == ELEM_QROTV) && (i == 1)) begin // quantum qvproc
                        // QRV op2 is a 32-bit angle stream paired against e8 indices. Reload the
                        // next 128-bit register chunk every four indices instead of every sixteen. // quantum qvproc
                        if ((state_q.qrotv_startup ? qrotv_startup_phase_next : state_next.count.val[1:0]) == 2'b00) begin // quantum qvproc
                            op_load_next[i] = OP_ALWAYS_VREG[i] | state_next.op_flags[i].vreg; // quantum qvproc
                        end // quantum qvproc
                        // One full 32-bit angle is consumed every cycle. // quantum qvproc
                        op_shift_next[i] = 1'b1; // quantum qvproc
                    end else begin
            ...
            if ((state_next.unit == UNIT_ELEM) && (state_next.mode.elem.op == ELEM_QROTV) &&
                state_q.qrotv_startup && (i != 1)) begin // quantum qvproc
                // During QRV startup prefetch only the 32-bit angle operand is allowed to advance.
                // Keep all other operands parked so the first visible event still starts from the
                // first logical element once the angle chunk is ready. // quantum qvproc
                op_shift_next[i] = 1'b0; // quantum qvproc
            end // quantum qvproc
        end
    ...
    always_comb begin
        ...
        unpack_ctrl.lmul      = state_q.lmul; // quantum qvproc
        unpack_ctrl.emul      = state_q.emul;
        unpack_ctrl.vxrm      = state_q.vxrm;
        unpack_ctrl.quantum_elem2_raw = state_q.quantum_elem2_raw; // quantum qvproc
        unpack_ctrl.quantum_elem3_raw = state_q.quantum_elem3_raw; // quantum qvproc
        unpack_ctrl.qrotv_startup = state_q.qrotv_startup; // quantum qvproc
    ...
    assign quantum_qrotv_op2_chunk_o = state_q.qrotv_op2_chunk; // quantum qvproc
    ...
    logic [OP_CNT   -1:0]               unpack_op_eew_override; // quantum qvproc
    cfg_vsew [OP_CNT   -1:0]            unpack_op_eew; // quantum qvproc
    always_comb begin
        ...
        // QRV needs mixed-width pairing: elem op2 (pipe_in_op_data_i[1]) advances in e32 while
        // elem op1 (pipe_in_op_data_i[0]) stays at the instruction's primary e8 width. // quantum qvproc
        if (UNITS[UNIT_ELEM] && (state_q.unit == UNIT_ELEM) && (state_q.mode.elem.op == ELEM_QROTV)) begin // quantum qvproc
            unpack_op_eew_override[1] = 1'b1; // quantum qvproc
            unpack_op_eew[1]          = VSEW_32; // quantum qvproc
        end
    end
    vproc_vregunpack #(
        ...
        .pipe_in_qrotv_startup_i ( unpack_ctrl.qrotv_startup ), // quantum qvproc
        .pipe_in_op_eew_override_i ( unpack_op_eew_override  ), // quantum qvproc
        .pipe_in_op_eew_i     ( unpack_op_eew                ), // quantum qvproc
        ...
        .qrotv_op2_ready_o    ( qrotv_op2_ready             ), // quantum qvproc
    ...
    logic                                   quantum_valid_raw; // quantum qvproc
    logic      [4:0]                        quantum_op_raw; // quantum qvproc
    logic      [XIF_ID_W              -1:0] quantum_instr_id_raw; // quantum qvproc
    logic      [4:0]                        quantum_vd_addr_raw; // quantum qvproc
    logic      [31:0]                       quantum_elem1_raw; // quantum qvproc
    logic      [31:0]                       quantum_elem2_raw_out; // quantum qvproc
    logic      [31:0]                       quantum_elem3_raw_out; // quantum qvproc
    vproc_pkg::cfg_lmul                     quantum_lmul_raw; // quantum qvproc
    vproc_pkg::cfg_emul                     quantum_emul_raw; // quantum qvproc
    logic      [2:0]                        quantum_count_mul_raw; // quantum qvproc
    logic                                   quantum_first_cycle_raw; // quantum qvproc
    logic                                   quantum_last_cycle_raw; // quantum qvproc
    logic                                   quantum_vl_part_0_raw; // quantum qvproc
    logic      [4:0]                        quantum_res_vaddr_raw; // quantum qvproc
    logic                                   quantum_first_valid_result_raw; // quantum qvproc
    logic                                   quantum_has_valid_result_raw; // quantum qvproc

    assign qrotv_startup_done = qrotv_op2_ready; // quantum qvproc
    ...
    /*
     * The quantum trace path is forwarded alongside the normal unit output
     * path. The pipeline itself does not reinterpret these fields; it only
     * preserves the slice-aligned information generated by the ELEM unit.
     */
    ...
    vproc_unit_mux #(
        ...
        .quantum_valid_o           ( quantum_valid_raw          ), // quantum qvproc
        .quantum_op_o              ( quantum_op_raw             ), // quantum qvproc
        .quantum_instr_id_o        ( quantum_instr_id_raw       ), // quantum qvproc
        .quantum_vd_addr_o         ( quantum_vd_addr_raw        ), // quantum qvproc
        .quantum_elem1_o           ( quantum_elem1_raw          ), // quantum qvproc
        .quantum_elem2_o           ( quantum_elem2_raw_out      ), // quantum qvproc
        .quantum_elem3_o           ( quantum_elem3_raw_out      ), // quantum qvproc
        .quantum_lmul_o            ( quantum_lmul_raw           ), // quantum qvproc
        .quantum_emul_o            ( quantum_emul_raw           ), // quantum qvproc
        .quantum_count_mul_o       ( quantum_count_mul_raw      ), // quantum qvproc
        .quantum_first_cycle_o     ( quantum_first_cycle_raw    ), // quantum qvproc
        .quantum_last_cycle_o      ( quantum_last_cycle_raw     ), // quantum qvproc
        .quantum_vl_part_0_o       ( quantum_vl_part_0_raw      ), // quantum qvproc
        .quantum_res_vaddr_o       ( quantum_res_vaddr_raw      ), // quantum qvproc
        .quantum_first_valid_result_o ( quantum_first_valid_result_raw ), // quantum qvproc
        .quantum_has_valid_result_o   ( quantum_has_valid_result_raw   ) // quantum qvproc
    );

    assign quantum_valid_o = quantum_valid_raw; // quantum qvproc
    assign quantum_op_o                 = quantum_op_raw; // quantum qvproc
    assign quantum_instr_id_o           = quantum_instr_id_raw; // quantum qvproc
    assign quantum_vd_addr_o            = quantum_vd_addr_raw; // quantum qvproc
    assign quantum_elem1_o              = quantum_elem1_raw; // quantum qvproc
    assign quantum_elem2_o              = quantum_elem2_raw_out; // quantum qvproc
    assign quantum_elem3_o              = quantum_elem3_raw_out; // quantum qvproc
    assign quantum_lmul_o               = quantum_lmul_raw; // quantum qvproc
    assign quantum_emul_o               = quantum_emul_raw; // quantum qvproc
    assign quantum_count_mul_o          = quantum_count_mul_raw; // quantum qvproc
    assign quantum_first_cycle_o        = quantum_first_cycle_raw; // quantum qvproc
    assign quantum_last_cycle_o         = quantum_last_cycle_raw; // quantum qvproc
    assign quantum_vl_part_0_o          = quantum_vl_part_0_raw; // quantum qvproc
    assign quantum_res_vaddr_o          = quantum_res_vaddr_raw; // quantum qvproc
    assign quantum_first_valid_result_o = quantum_first_valid_result_raw; // quantum qvproc
    assign quantum_has_valid_result_o   = quantum_has_valid_result_raw; // quantum qvproc
