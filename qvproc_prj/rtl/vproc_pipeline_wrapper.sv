...
`endif
    ...
    always_comb begin
        ...
        unique case (pipe_in_data_i.mode.elem.op)
            ...
            ELEM_QSINGLE:   begin // quantum qvproc
                // Quantum single op does not require post-op flush.
                elem_flush        = 1'b0; // quantum qvproc
                // Quantum single op consumes regular vector data from vs2-path.
                elem_vs2_data     = 1'b1; // quantum qvproc
                // Quantum single op does not consume vs2 as mask bits.
                elem_vs2_mask     = 1'b0; // quantum qvproc
                // Quantum single op does not use dynamic gather-style addressing.
                elem_vs2_dyn_addr = 1'b0; // quantum qvproc
            end
            ELEM_QPAIR:     begin // quantum qvproc
                // Quantum pair op does not require post-op flush.
                elem_flush        = 1'b0; // quantum qvproc
                // Quantum pair op consumes regular vector data from vs2-path.
                elem_vs2_data     = 1'b1; // quantum qvproc
                // Quantum pair op does not consume vs2 as mask bits.
                elem_vs2_mask     = 1'b0; // quantum qvproc
                // Quantum pair op does not use dynamic gather-style addressing.
                elem_vs2_dyn_addr = 1'b0; // quantum qvproc
            end
            ELEM_QROTG:     begin // quantum qvproc
                // Quantum global-rotation op does not require post-op flush.
                elem_flush        = 1'b0; // quantum qvproc
                // Quantum global-rotation op still needs vector element data.
                elem_vs2_data     = 1'b1; // quantum qvproc
                // Quantum global-rotation op does not consume vs2 as mask bits.
                elem_vs2_mask     = 1'b0; // quantum qvproc
                // Quantum global-rotation op does not use dynamic gather-style addressing.
                elem_vs2_dyn_addr = 1'b0; // quantum qvproc
            end
            ELEM_QROTV:     begin // quantum qvproc
                // Quantum variable-rotation op does not require post-op flush.
                elem_flush        = 1'b0; // quantum qvproc
                // Quantum variable-rotation op needs vector data and angle operand data.
                elem_vs2_data     = 1'b1; // quantum qvproc
                // Quantum variable-rotation op does not consume vs2 as mask bits.
                elem_vs2_mask     = 1'b0; // quantum qvproc
                // Quantum variable-rotation op does not use dynamic gather-style addressing.
                elem_vs2_dyn_addr = 1'b0; // quantum qvproc
            end
            default: ;
        endcase
    ...
    always_comb begin
        ...
        state_init.lmul           = pipe_in_data_i.lmul; // quantum qvproc
        ...
        state_init.quantum_elem2_raw = pipe_in_data_i.quantum_elem2_raw; // quantum qvproc
        state_init.quantum_elem3_raw = pipe_in_data_i.quantum_elem3_raw; // quantum qvproc
    ...
    /*
     * The wrapper specializes operand/result shapes for a configured lane, but
     * the quantum trace itself remains a straight-through export from the
     * instantiated `vproc_pipeline`. Using the same signal names keeps the
     * repeated specialization blocks transparent to the new output path.
     */
    generate
