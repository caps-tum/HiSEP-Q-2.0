...
module vproc_vregunpack
    #(
        ...
        input  logic                                  pipe_in_qrotv_startup_i, // quantum qvproc
        input  logic   [OP_CNT-1:0]                   pipe_in_op_eew_override_i, // quantum qvproc
        input  vproc_pkg::cfg_vsew [OP_CNT-1:0]       pipe_in_op_eew_i,     // quantum qvproc
        ...
        output logic                                  qrotv_op2_ready_o,    // quantum qvproc
    ...
    typedef struct packed {
        ...
        logic   [OP_CNT-1:0]                  op_eew_override; // quantum qvproc
        cfg_vsew [OP_CNT-1:0]                 op_eew; // quantum qvproc
        logic   [OP_CNT-1:0]                  op_load;
        logic   [OP_CNT-1:0][MAX_VADDR_W-1:0] op_vaddr;
        FLAGS_T [OP_CNT-1:0]                  op_flags;
        logic   [OP_CNT-1:0][31           :0] op_xval;
        logic   [OP_CNT-1:0][MAX_VPORT_W-1:0] op_buffer;
        logic   [OP_CNT-1:0]                  op_buffer_valid; // quantum qvproc
        logic   [OP_CNT-1:0][MAX_OP_W   -1:0] op_data;
        logic   [OP_CNT-1:0]                  op_data_valid; // quantum qvproc
    } vregunpack_state_t;
    ...
    always_comb begin
        ...
        stage_0.qrotv_startup = pipe_in_qrotv_startup_i; // quantum qvproc
        stage_0.op_eew_override = pipe_in_op_eew_override_i; // quantum qvproc
        stage_0.op_eew          = pipe_in_op_eew_i; // quantum qvproc
        ...
        stage_0.op_buffer_valid = '0; // quantum qvproc
        stage_0.op_data_valid   = '0; // quantum qvproc
    end
    ...
    logic [OP_CNT-1:0][MAX_OP_W   -1:0] op_data;
    logic [OP_CNT-1:0]                  op_output_valid; // quantum qvproc
            ...
            if (stage_ready[i]) begin
                ...
                for (int j = 0; j < OP_CNT; j++) begin
                    if (i == OP_STAGE[j] + 1) begin
                        if (stage_valid_q[OP_STAGE[j]]) begin
                            ...
                            stage_state_d[i].op_buffer_valid[j] =
                                stage_state_q[i].op_buffer_valid[j] | stage_state[OP_STAGE[j]].op_load[j]; // quantum qvproc
                        end else begin
                            stage_state_d[i].op_buffer[j] = stage_state_q[i].op_buffer[j];
                            stage_state_d[i].op_buffer_valid[j] = stage_state_q[i].op_buffer_valid[j]; // quantum qvproc
                        end
                    end
                end
                ...
                for (int j = 0; j < OP_CNT; j++) begin
                    if (i == OP_STAGE[j] + 2) begin
                        ...
                        stage_state_d[i].op_data_valid[j] = stage_state_q[OP_STAGE[j] + 1].op_buffer_valid[j]; // quantum qvproc
                    end
                end
            end
        end
    ...
    logic qrotv_out_hold; // quantum qvproc
    always_comb begin
        qrotv_out_hold = 1'b0; // quantum qvproc
        // This hold is only meant to hide QRV startup cycles until the first e32 angle chunk is
        // actually available at the unpack output. Other instructions may also override an
        // operand EEW to 32 bits (for example QSG's scalar rs2 payload), but they must not be
        // stalled by the QRV-specific warm-up rule. // quantum qvproc
        if (stage_valid[UNPACK_STAGES] && stage_state[UNPACK_STAGES].qrotv_startup) begin // quantum qvproc
            for (int i = 0; i < OP_CNT; i++) begin // quantum qvproc
                if (stage_state[UNPACK_STAGES].op_eew_override[i] &&
                    (stage_state[UNPACK_STAGES].op_eew[i] == VSEW_32)) begin // quantum qvproc
                    if (OP_STAGE[i] + 1 == UNPACK_STAGES) begin // quantum qvproc
                        qrotv_out_hold |= ~stage_state[OP_STAGE[i] + 1].op_buffer_valid[i]; // quantum qvproc
                    end else begin // quantum qvproc
                        qrotv_out_hold |= ~stage_state[UNPACK_STAGES].op_data_valid[i]; // quantum qvproc
                    end // quantum qvproc
                end // quantum qvproc
            end // quantum qvproc
        end // quantum qvproc
    ...
    always_comb begin
        pipe_in_ready_o    = stage_ready[0] & ~qrotv_out_hold;
        pipe_out_valid_o   = stage_valid[UNPACK_STAGES] & ~qrotv_out_hold;
    ...
    always_comb begin
        op_output_valid = '0; // quantum qvproc
        for (int i = 0; i < OP_CNT; i++) begin // quantum qvproc
            if (OP_STAGE[i] + 1 == UNPACK_STAGES) begin // quantum qvproc
                op_output_valid[i] = stage_valid[UNPACK_STAGES] &
                                     stage_state[OP_STAGE[i] + 1].op_buffer_valid[i]; // quantum qvproc
            end else begin // quantum qvproc
                op_output_valid[i] = stage_valid[UNPACK_STAGES] &
                                     stage_state[UNPACK_STAGES].op_data_valid[i]; // quantum qvproc
            end // quantum qvproc
        end // quantum qvproc
    end // quantum qvproc
...
`endif // quantum qvproc
    ...
    generate
        for (genvar i = 0; i < OP_CNT; i++) begin
            ...
            if (OP_DYN_ADDR[i]) begin
                ...
                if (ADDR_STAGE > 0) begin
                    // QRV op2 reload can update the source vreg address one cycle ahead of the
                    // buffered load stage. When that operand carries its own EEW override (used by
                    // QRV to fetch e32 angles while the instruction itself runs at e8), the newest
                    // address lives in the prior unpack stage even though op_load is asserted in
                    // the current one. Use that newer address so the actual vreg read port follows
                    // the chunk switch instead of re-reading the previous block. // quantum qvproc
                    assign op_vreg_addr[i] =
                        (stage_state[ADDR_STAGE].op_eew_override[i] &&
                         (stage_state[ADDR_STAGE].op_eew[i] == VSEW_32)) ?
                            stage_state[ADDR_STAGE-1].op_vaddr[i] :
                            stage_state[ADDR_STAGE].op_vaddr[i];
                end else begin
    ...
    always_comb begin
        for (int i = 0; i < OP_CNT; i++) begin
            ...
            op_load_eew     [i] = stage_state[OP_STAGE[i]    ].op_eew_override[i] ? stage_state[OP_STAGE[i]    ].op_eew[i] : stage_state[OP_STAGE[i]    ].eew; // quantum qvproc
            op_buffer       [i] = stage_state[OP_STAGE[i] + 1].op_buffer[i];
            op_extract_flags[i] = stage_state[OP_STAGE[i] + 1].op_flags[i];
            op_extract_eew  [i] = stage_state[OP_STAGE[i] + 1].op_eew_override[i] ? stage_state[OP_STAGE[i] + 1].op_eew[i] : stage_state[OP_STAGE[i] + 1].eew; // quantum qvproc
        ...
        for (genvar i = 0; i < OP_CNT; i++) begin
            ...
            always_comb begin
                ...
                if ((~OP_MASK[i] & ~OP_NARROW[i] & ~OP_ALLOW_ELEMWISE[i] & ~OP_ALWAYS_ELEMWISE[i]) |
                    ...
                    if (~qrotv_startup_hold) begin // quantum qvproc
