...
module vproc_unit_wrapper import vproc_pkg::*; #(
    ...
    generate
        if (UNIT == UNIT_LSU) begin
            /*
             * Only the ELEM unit produces the exported quantum slice stream.
             * Non-ELEM wrappers tie the trace outputs off so the added ports
             * stay behaviorally inert for LSU traffic.
             */
             ...
             * Only the ELEM unit produces the exported quantum slice stream.
             * Non-ELEM wrappers tie the trace outputs off so the added ports
             * stay behaviorally inert for ALU traffic.
             */
            assign quantum_valid_o       = 1'b0; // quantum qvproc
            assign quantum_op_o          = '0; // quantum qvproc
            assign quantum_instr_id_o    = '0; // quantum qvproc
             ...
             * Only the ELEM unit produces the exported quantum slice stream.
             * Non-ELEM wrappers tie the trace outputs off so the added ports
             * stay behaviorally inert for MUL traffic.
             */
            assign quantum_valid_o       = 1'b0; // quantum qvproc
            assign quantum_op_o          = '0; // quantum qvproc
            assign quantum_instr_id_o    = '0; // quantum qvproc
             ...
             * Only the ELEM unit produces the exported quantum slice stream.
             * Non-ELEM wrappers tie the trace outputs off so the added ports
             * stay behaviorally inert for SLD traffic.
             */
            assign quantum_valid_o       = 1'b0; // quantum qvproc
            assign quantum_op_o          = '0; // quantum qvproc
            assign quantum_instr_id_o    = '0; // quantum qvproc
            ...
            /*
             * The ELEM wrapper is the natural tap point for quantum slice
             * export because it still has one-result-per-element visibility
             * before vreg packing merges those slices into wider writes.
             */
            ...
            vproc_elem #(
                ...
                .quantum_lmul_o        ( quantum_lmul_o                 ), // quantum qvproc
